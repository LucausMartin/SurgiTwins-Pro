...
class VTKOverlayWindow(QVTKRenderWindowInteractor):
    ...
    def __init__(self,
                 ...
                 opencv_style=True,
                 init_pose=False,
                 reset_camera=False,     # 原本为True，我修改为了False
                ):
        """
        ...
        self.camera_to_world = new_camera_to_world
        ...
        if init_pose:
            ...
            print("---------------------------------------------------self.set_camera_pose(default_pose)")
    ...
    def set_video_image(self, input_image):
        ...
        # print("self.input.shape, input_image.shape:", self.input.shape, input_image.shape)
    ...
    def __update_projection_matrix(self):
        ...
        opengl_mat = None
        vtk_mat = None
        print("self.camera_matrix:", self.input.shape, self.camera_matrix, self.clipping_range)
        if self.camera_matrix is not None:
            ...
            print("vpx, vpy, vpw, vph:", vpx, vpy, vpw, vph)
    ...
    def add_vtk_models(self, models, layer=1):
        ...
        print("不会自动更新相机，ini_camera_properties:", self.get_camera_state())
        print("打印相机的位置，即外参，self.camera_to_world:", self.camera_to_world)
        if self.reset_camera:
            print("---self.reset_camera:", self.reset_camera)
            print("---ini_camera_properties:", self.get_camera_state())
            renderer.ResetCamera()
            ini_camera_properties = self.get_camera_state()
            print("ini_camera_properties:")
            print(ini_camera_properties)
            renderer.SetViewport(0.0, 0.0, 1, 1)  # 默认视口：全屏
