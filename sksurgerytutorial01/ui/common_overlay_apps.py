...
from PySide2.QtWidgets import QWidget, QHBoxLayout, QDesktopWidget, QVBoxLayout, QPushButton
...
from .vtk_overlay_window import VTKOverlayWindow
from .ControlSlider import ControlSlider
from .TransparencySlider import TransparencySlider
...
class OverlayBaseWidget(QWidget):
    ...
    def __init__(self, video_source, dims=None):
        ...
        # 添加 vtk_overlay_window
        self.vtk_overlay_window = VTKOverlayWindow()
        self.mainlayout.addWidget(self.vtk_overlay_window)
        ...
        self.x_move_slider = ControlSlider(label_text="平移x值： ",min_val=-100,max_val=100,default_val=0)
        self.x_move_slider.setFixedWidth(screen_geometry.width()//4)
        self.y_move_slider = ControlSlider(label_text="平移y值： ",min_val=-100,max_val=100,default_val=0)
        self.y_move_slider.setFixedWidth(screen_geometry.width()//4)
        self.z_move_slider = ControlSlider(label_text="平移z值： ",min_val=-100,max_val=100,default_val=0)
        self.z_move_slider.setFixedWidth(screen_geometry.width()//4)

        self.layout2.addWidget(self.x_move_slider,alignment=Qt.AlignCenter)
        self.layout2.addWidget(self.y_move_slider,alignment=Qt.AlignCenter)
        self.layout2.addWidget(self.z_move_slider,alignment=Qt.AlignCenter)


        # 添加控制滑块
        self.layout3=QVBoxLayout()
        self.x_slider = ControlSlider(label_text="绕X轴旋转： ",min_val=-180,max_val=180,default_val=0)
        self.x_slider.setFixedWidth(screen_geometry.width()//4)
        self.y_slider = ControlSlider(label_text="绕Y轴旋转： ",min_val=-180,max_val=180,default_val=0)
        self.y_slider.setFixedWidth(screen_geometry.width()//4)
        self.z_slider = ControlSlider(label_text="绕Z轴旋转： ",min_val=-180,max_val=180,default_val=0)
        self.z_slider.setFixedWidth(screen_geometry.width()//4)


        # 添加滑块到布局,并设置居中
        self.layout3.addWidget(self.x_slider,alignment=Qt.AlignCenter)
        self.layout3.addWidget(self.y_slider,alignment=Qt.AlignCenter)
        self.layout3.addWidget(self.z_slider,alignment=Qt.AlignCenter)

        # 添加透明度滑块
        self.transparency_slider = TransparencySlider(label_text="透明度： ", min_val=0, max_val=100, default_val=30)
        self.transparency_slider.setFixedWidth(screen_geometry.width() // 4)

        # 添加到主布局
        self.layout1.addLayout(self.layout2)
        self.layout1.addLayout(self.layout3)
        self.layout1.addWidget(self.transparency_slider,alignment=Qt.AlignCenter)
        ...
        # 添加显示按钮
        self.on_off_button = QPushButton("显示/隐藏")
        self.on_off_button.setFixedSize(screen_geometry.width()//5,50)
        self.on_off_button.setFont(self.button_font)
        self.layout1.addWidget(self.on_off_button,alignment=Qt.AlignCenter)

        # 添加更新自动对齐按钮
        self.auto_align_button = QPushButton("自动对齐")
        self.auto_align_button.setFixedSize(screen_geometry.width()//5,50)
        self.auto_align_button.setFont(self.button_font)
        self.layout1.addWidget(self.auto_align_button,alignment=Qt.AlignCenter)
